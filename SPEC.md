...
- Padsパネルのグリップ・タイトル・グリッドサイズ選択・編集ボタンは常時
  表示の`.pad-panel-bar`に配置し、パネルを格納していても操作できる
  （`<button>`の入れ子を避けるため、ドラッグ/タップ判定は`.pad-panel-
  tap-zone`のみに絞り、グリッドサイズ選択と編集ボタンは兄弟要素にした）
