...
- モバイルのPadsパネルは、タップだけでなく上下スワイプでも開閉でき、指の
  ...
  開閉に必要なドラッグ量は`panelDragRange`（実際のパネル高さでなく短い
  固定距離）で正規化しており、展開・格納どちらも同じ指の移動量で反応する
- Padsパネルは常時表示の`.pad-panel-bar`を持ち、パネルを格納していても
  グリッドサイズ選択・編集ボタンを操作できる。上段にドラッグ/タップ判定
  専用の中央寄せグリップ行（`.pad-panel-grip-row`、paddingは最小限）、
  その直下にグリッドサイズ選択（旧「Pads」の文字の代わり）・Clear All・
  編集ボタンを並べた行（`.pad-panel-controls-row`）を配置（`<button>`の
  入れ子を避けるため、ドラッグ/タップ判定はグリップ行のみに絞った）
- パッド編集モードのドラッグは、動かしたパッドと着地先のパッドだけを
  入れ替える単純なswapにした（以前は配列の挿入によるシフトで、無関係な
  他のパッドまで位置がずれていた）。ドラッグ中は現在ホバー中のスロットを
  「ドロップ先プレビュー」としてハイライトし、離す前にどこへ入るか分かる
  ようにした。ドラッグ元のスロットは空のプレースホルダーとして表示し、
  実体（コード内容）は`.floating-drag`として画面に固定表示しつつ指に追従
- ダイアトニックパレット/Chord Builderの「+」からパッドに追加すると、
  クリックした位置から追加先のパッド位置へ飛んでいくアニメーション
  （`.flying-chip`、FLIP風に開始位置→終了位置へCSSトランジション）
- 編集完了ボタンはチェックマークアイコン（鉛筆⇔チェックのトグル）
- パッドの×削除ボタンはアイコンを中央揃えに修正（以前は右上にずれていた）
- ポインタードラッグ系の処理（パッド並べ替え・パネルのスワイプ）は
  `pointerId`でフィルタし、無関係なポインターイベント（別のマウス/タッチ
  や自動テスト由来のもの）が同時に来てもドラッグ量が乱れないようにした。
  また、パネル開閉判定は「トグル」ではなく、ドラッグ開始時点の状態から
  導いた絶対値を`setPadsCollapsed()`に渡す方式にして、万一ハンドラが
  重複発火しても打ち消し合わないようにしている
